...
//answer    
function pickIt(arr) {
    //variable that hold an array of odd or even numbers
    var odd = [], even = [];
    //loop that places numbers from the given arr into the odd or even array
    for (i = 0; i < arr.length; i++) {
        arr[i] % 2 ? odd.push(arr[i]) : even.push(arr[i])
    }
    //return the odd and even arrays in an array
    return [odd, even];
}