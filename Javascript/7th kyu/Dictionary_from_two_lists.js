...
function createDict(keys, values) {
    ...
    const obj = {}
    //loop that goes through all the keys and places the key value pair into the obj
    keys.forEach((x, i) => obj[x] = values[i] == '' || values[i] || values[i] == false ? values[i] : null)
    //returned the final obj             
    return obj
}