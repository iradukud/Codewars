...
function minimumSteps(numbers, value) {
    //variable that holds the sort array of original numbers, ascendingly
    const sort = numbers.sort((a, b) => a - b)
    //variiable that holds the sum of the elements
    let sum = 0
    for (i = 0; i < sort.length; i++) {
        sum += sort[i]
        if (sum >= value) {
            //if the sum is greater or equal to the value, return i
            return i
        }
    }
}