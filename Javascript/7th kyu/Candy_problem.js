...
function candies(kids) {
    //if the one or no kids show up return negative 1
    if (kids.length == 0 || kids.length == 1) { return -1 }
    //variable that holds the highest value from the kids
    const max = Math.max(...kids)
    //return the amount of candies given out to make the kids have the same amount of candies
    return [...kids].reduce((sum, num) => sum + (max - num), 0)
}