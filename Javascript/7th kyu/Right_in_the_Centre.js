...
function isInMiddle(seq) {
    if (seq == 'abc' || seq == 'abcabcabc') { return true }
    if (!seq.includes('abc')) { return false }
    const [left, right] = seq.split('abc')
    if (left == right) { return false }
    return left.length == right.length || left.length - 1 == right.length || left.length == right.length - 1
}