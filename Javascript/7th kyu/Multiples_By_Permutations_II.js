...
function findLowestInt(k, n = 1) {
    //loop that run as long as n is less or equal to the nax safe integer
    while (n <= Number.MAX_SAFE_INTEGER) {
        //times k to the current n and convert to a string
        let k1 = n * k + ''
        //times k+1 to the current n and convert to a string
        let k2 = n * (k + 1) + ''
        //if the how the same digit return n and end the loop
        if ([...k1].sort().join('') === [...k2].sort().join('')) {
            return n
        }
        //add to n adn go to the next iteration
        n++
    }
}