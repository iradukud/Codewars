...
//Solution  
function solve(a, b) {
    //variable that holds each players final score
    const [x, y] = a.reduce((p, a, i) => a == b[i] ? p : (++p[a > b[i] ? 0 : 1], p), [0, 0]);
    //variable that holds the message that will be returned
    const m = (x == y) ? 'that looks like a "draw"! Rock on!' : (x > y) ? 'Alice made "Kurt" proud!' : 'Bob made "Jeff" proud!';
    //returned the final score with the corresponding message
    return `${x}, ${y}: ${m}`;
  }