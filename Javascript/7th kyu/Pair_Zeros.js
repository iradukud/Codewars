...
function pairZeros(arr) {
    //variable that holds the array which will be returned 
    const arrz = []
    //variable that keeps count of the current zero pairs
    let count = 1
    //loop that goes though the whole arr and places elements into arrz
    arr.forEach(x => {
        if (x == 0 && count < 2) {
            arrz.push(x)
            count++
