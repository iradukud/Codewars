...
function foldTo(distance) {
    //if the given distance is lower than zero return null
    if (distance < 0) { return null }
    //variable that keeps count of the amount of fold need
    let counter = 0
    //loop that countinues to fold the paper till it reaches and surpasses the distance 
    for (var i = 0.0001; i < distance; i *= 2) {
        counter += 1
    }
    //return the amout of times the paper was folded
    return counter
}