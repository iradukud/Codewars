...
function newAvg(arr, newavg) {
    //determined the next donation required  
    const nextDon = (arr.length + 1) * newavg - arr.reduce((sum, num) => sum + num, 0)
    //if it was zero or less return error message  
    if (nextDon <= 0) { throw 'Nope' }
    //else returun the next donation rounded up
    return Math.ceil(nextDon)
}