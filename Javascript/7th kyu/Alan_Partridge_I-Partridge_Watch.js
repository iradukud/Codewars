...
//Solution  
function part(x) {
    //variable that holds the related items
    const related = ['Partridge', 'PearTree', 'Chat', 'Dan', 'Toblerone', 'Lynn', 'AlphaPapa', 'Nomad']
    //returned the appropriate message depending on whether the given array contained any related of the terms
    return x.some(x => related.includes(x)) ? 'Mine\'s a Pint' + x.filter(x => related.includes(x)).map(x => x = '!').join('') : 'Lynn, I\'ve pierced my foot on a spike!!'
}