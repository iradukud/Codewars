...
function decodeResistorColors(bands) {
    //variable that holds the replacement values for the bands
    const band = { 'black': '0', 'brown': '1', 'red': '2', 'orange': '3', 'yellow': '4', 'green': '5', 'blue': '6', 'violet': '7', 'gray': '8', 'white': '9' }
    //variable that holds the replacement values for the resistance
    const tolerV = { 'gold': "5%", 'silver': "10%" }
    //variable that holds the resist values in an array
    const arr = bands.split(' ').map(x => band[x] ? band[x] : x)
    //variable that holds the total resistance
    const resist = (arr[0] + arr[1]) * (10 ** arr[2])
    //variable that holds the tolerance
    const toler = arr.length == 4 ? tolerV[arr[3]] : '20%'
    //returned the resistance and tolerance in a appropriate string message
    return resist < 1000 ? `${resist} ohms, ${toler}` : resist < 1000000 ? `${resist / 1000}k ohms, ${toler}` : `${resist / 1000000}M ohms, ${toler}`
}