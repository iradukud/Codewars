...
function momentOfTimeInSpace(moment) {
    //got the value of time by split the string into an array then filtered out numbers from 1-9
    //then reduce the filtered array to get it's sum
    const time = moment.split('').filter(x => x >= 1 && x <= 9).reduce((sum, num) => sum + (+num), 0)
    //got the value of time by deleting every character that would be use n the time's calculation
    //then getting the length of the new string
    const space = moment.replace(/[1-9]/g, '').length
    //return an array that has 3 boolean values which correspond to past, present and future respectively
    //the boolean is obtain by comparing time and space variables
    return [time < space, space == time, time > space]
}