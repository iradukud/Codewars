...
function conferencePicker(citiesVisited, citiesOffered) {
    //variable that will hold the potential places she could visit 
    const arr = []
    //loop that goes through all the offered cities
    citiesOffered.forEach(x => !citiesVisited.includes(x) ? arr.push(x) : null)
    //if none return an error message
    //else return the first option in the potential places she can visit
    return arr.length == 0 ? 'No worthwhile conferences this year!' : arr[0]
}