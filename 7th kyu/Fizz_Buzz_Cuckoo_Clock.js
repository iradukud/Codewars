...
function fizzBuzzCuckooClock(time) {
    //variable that holds the minute of the given time
    const minute = time.split(':')[1]
    //variable that holds the hour of the given time
    const hour = time.split(':')[0] >= 13 ? time.split(':')[0] - 12 : time.split(':')[0] == 0 ? 12 : time.split(':')[0]
    //returned the corresponding string depending on the minute and hours of the time given
    return minute == 0 || minute == 12 ? Array.from({ length: hour }, _ => 'Cuckoo').join(' ') : minute == 30 ? 'Cuckoo' : minute % 3 == 0 && minute % 5 == 0 ? 'Fizz Buzz' : minute % 3 == 0 ? 'Fizz' : minute % 5 == 0 ? 'Buzz' : 'tick'
}