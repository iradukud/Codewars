...
function barista(coffees) {
    //sorted the coffee brew times from low to highest
    const coffeeSorted = coffees.sort((a, b) => a - b)
    //created an array that houses the wait times
    let waitTimes = []
    //variable that holds the individual wait times
    let waitTime = 0
    //loop through all the coffee brew times and determine the total waiting time for each
    //the place it in the array that holds all the wait times
    for (i = 0; i < coffees.length; i++) {
        if (i == 0) {
            waitTime = coffeeSorted[i]
        } else {
            waitTime += coffeeSorted[i] + 2
        }
        waitTimes.push(waitTime)
    }
    //return the sum of the wait  times
    return waitTimes.reduce((sum, num) => sum + num, 0)
}
