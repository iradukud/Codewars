...
function goto(level, button) {
    //if any dangerous arguments are passed return 0
    if (level > 3 || level < 0 || Math.abs(button) > 3 || typeof level != 'number' || typeof button != 'string' || !Number.isInteger(level)) { return 0 }
    //else return the amount of space the elevate will move 
    return button - level
}