...
function ballTest(s, r) {
    //variable the holds the amount of cracks in the given length(s)
    const crack = [...r.slice(0, s)].filter(x => x == 'x').length
    //if s is less than zero return false
    if (s < 0) {
        return false
    }
    //if s is greater or equal to r's length return true
    if (s >= r.length) {
        return true
        //else pass the new values back into the function (recursion)
    } else {
        return ballTest(s - 1 - crack, r.slice(s))
    }
}