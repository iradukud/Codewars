...
function getScore(arr) {
    //variable that holds the lines cleared and corresponding score
    const points = { 1: 40, 2: 100, 3: 300, 4: 1200 }
    //variable that holds the total score
    let score = 0
    //variable that holds the current count of lines cleared
    let lines = 0
    //loop that goes through the array
    for (const n of arr) {
        //added to the score
        score += (points[n] || 0) * (1 + (lines / 10 | 0))
        //increased the lines  
        lines += n
    }
    //returned final score
    return score
}