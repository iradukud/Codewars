...
function solution(frank, sam, tom) {
    if (sam.filter(x => x >= 11).length == 2 || tom.filter(x => x >= 11).length == 2) { return false }

    frank.sort((a, b) => a - b)
    const f1 = frank[frank.length - 1]
    const f2 = frank[frank.length - 2]

    return sam.filter(x => x < f1 || x < f2).length >= 2 && tom.filter(x => x < f1 || x < f2).length >= 2
}