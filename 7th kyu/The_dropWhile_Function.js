...
function dropWhile(arr, pred) {
    //loop that runs as long as the arr has elements  and pred on current element is true
    //if the first element meets the pred condition then remove the element
    while (arr.length && pred(arr[0])) {
        arr.shift()
    }
    //returned the modified final arr
    return arr
}