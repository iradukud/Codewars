...
//Solution  
Array.prototype.map = function (element) {
    let ans = []
    //loop that runs the length of the given array
    for (let i = 0; i < this.length; i++) {
        //how an operation done on each element and push it to the created array
        ans.push(element(this[i]))
    }
    //returned the final array with function done on it
    return ans
}