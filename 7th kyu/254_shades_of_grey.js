...
function shadesOfGrey(n) {
    //variable that holds all the shades of grey
    const shades = []
    //loop that places each shade hexa value into the array
    for (i = 1; i <= Math.min(n, 254); i++) {
        //convert the i to a Hexadecimal
        let grey = ('0' + i.toString(16)).slice(-2)
        shades.push('#' + grey + grey + grey)
    }
    //returned the array of the grey shades 
    return shades
}