...
function balancedNum(n) {
    ...
    if ((n + '').length % 2) {
        return [...(n + '')].slice(((n + '').length / 2) + 1).reduce((sum, num) => sum + +num, 1) == [...(n + '')].slice(0, (n + '').length / 2).reduce((sum, num) => sum + +num, 1) ? 'Balanced' : 'Not Balanced'
    } else {
        console.log([...(n + '')].slice(0, ((n + '').length / 2) - 1), [...(n + '')].slice(((n + '').length / 2) + 1))
        return [...(n + '')].slice(0, ((n + '').length / 2) - 1).reduce((sum, num) => sum + +num, 1) == [...(n + '')].slice(((n + '').length / 2) + 1).reduce((sum, num) => sum + +num, 1) ? 'Balanced' : 'Not Balanced'
    }
}