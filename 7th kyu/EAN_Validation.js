...
function validateEAN(eanCode) {
    //variable that holds the sum of the code modde by 10 
    const sum = [...eanCode].slice(0, 12).reduce((sum, num, i) => i % 2 ? sum + (num * 3) : sum + +num, 0) % 10
    //returned a boolean based on when the 12th position equals the checksum
